...
router.get('/', (_req: Request, res: Response) => {
  try {
    ...
    const galleryUseOriginals = getSetting('gallery_use_originals') === '1';
    ...
    res.json({ favorites_on_home: favoritesOnHome, favorites_sort_order: favoritesSortOrder, favorites_count: favoritesCount, gallery_use_originals: galleryUseOriginals });
  } catch (err) {
...
router.put('/', (req: Request, res: Response) => {
  try {
    const { favorites_on_home, favorites_sort_order, gallery_use_originals } = req.body;
    ...
    if (gallery_use_originals !== undefined) {
      setSetting('gallery_use_originals', gallery_use_originals ? '1' : '0');
    }
    res.json({ status: 'ok' });
  } catch (err) {
